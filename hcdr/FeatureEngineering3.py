...
class FeatureEngineering3:
    ...
    def get_dataset(self):
        ...
        app_train = pd.read_csv(os.path.join(self.default_dir, 'application_train.csv'))
        app_test = pd.read_csv(os.path.join(self.default_dir, 'application_test.csv'))
        apps = pd.concat([app_train, app_test])
        prev = pd.read_csv(os.path.join(self.default_dir, 'previous_application.csv'), dtype=prev_dtype)
        bureau = pd.read_csv(os.path.join(self.default_dir, 'bureau.csv'), dtype=bureau_dtype)
        bureau_bal = pd.read_csv(os.path.join(self.default_dir, 'bureau_balance.csv'), dtype=bureau_bal_dtype)
        pos_bal = pd.read_csv(os.path.join(self.default_dir, 'POS_CASH_balance.csv'), dtype=pos_dtype)
        install = pd.read_csv(os.path.join(self.default_dir, 'installments_payments.csv'), dtype=install_dtype)
        card_bal = pd.read_csv(os.path.join(self.default_dir, 'credit_card_balance.csv'), dtype=card_dtype)

        return apps, prev, bureau, bureau_bal, pos_bal, install, card_bal


    def get_apps_processed(self, apps):
        '''
    ...
    def get_prev_processed(self, prev):
        """
    ...
    def get_prev_refused_appr_agg(self, prev):
        '''
    ...
    def get_prev_agg(self, prev):
        '''
        Aggregation for previous credit
        '''
        prev = self.get_prev_processed(prev)
        prev = self.get_prev_processed(prev)
        prev_amt_agg = self.get_prev_amt_agg(prev)
        
        # Refused or Approved previous credit
        prev_refused_appr_agg = self.get_prev_refused_appr_agg(prev)
    ...
    def get_bureau_processed(self, bureau):
        '''
        feature engineering for bureau
        '''
        
        # DAYS_CREDIT_ENDDATE : CB Remaining duration of CB credit (in days) at the time of application in Home Credit
        # DAYS_ENDDATE_FACT : CB Days since CB credit ended at the time of application in Home Credit (only for closed credit)
    ...
    def get_bureau_day_amt_agg(self, bureau):
        '''
    ...
    def get_bureau_active_agg(self, bureau):
        '''
    ...
    def get_bureau_bal_agg(self, bureau, bureau_bal):
        '''
    ...
    def get_bureau_agg(self, bureau, bureau_bal):
        '''
        '''
        bureau = self.get_bureau_processed(bureau)
        bureau_day_amt_agg = self.get_bureau_day_amt_agg(bureau)
        bureau_active_agg = self.get_bureau_active_agg(bureau)
        bureau_bal_agg = self.get_bureau_bal_agg(bureau, bureau_bal)
        
        # bureau_day_amt_agg와 bureau_active_agg 
    ...
    def get_pos_bal_agg(self, pos_bal):
        '''
    ...
    def get_install_agg(self, install):
        """
        """
        # AMT_INSTALMENT : 
        # AMT_PAYMENT - 
    ...
    def get_card_bal_agg(self, card_bal):
        '''
        '''
    ...
    def get_apps_all_encoded(self, apps_all):
        """
        ...
        return apps_all
    def get_apps_all_train_test(self, apps_all):
        """
    ...
    def get_apps_all_with_all_agg(self, apps, prev, bureau, bureau_bal, pos_bal, install, card_bal):
        '''
        '''
        apps_all =  self.get_apps_processed(apps)
        prev_agg = self.get_prev_agg(prev)
        bureau_agg = self.get_bureau_agg(bureau, bureau_bal)
        pos_bal_agg = self.get_pos_bal_agg(pos_bal)
        install_agg = self.get_install_agg(install)
        card_bal_agg = self.get_card_bal_agg(card_bal)
