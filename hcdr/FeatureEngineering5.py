...
class FeatureEngineering5:
    ...
    def main(self, debug= False):
        ...
        with self.timer("application_train and application_test"):
            df = self.get_train_test(self.DATA_DIRECTORY, num_rows= num_rows)
            print("Application dataframe shape: ", df.shape)
        with self.timer("Bureau and bureau_balance data"):
            bureau_df = self.get_bureau(self.DATA_DIRECTORY, num_rows= num_rows)
            df = pd.merge(df, bureau_df, on='SK_ID_CURR', how='left')
            print("Bureau dataframe shape: ", bureau_df.shape)
            del bureau_df; gc.collect()
        with self.timer("previous_application"):
            prev_df = self.get_previous_applications(self.DATA_DIRECTORY, num_rows)
            df = pd.merge(df, prev_df, on='SK_ID_CURR', how='left')
            print("Previous dataframe shape: ", prev_df.shape)
            del prev_df; gc.collect()
        with self.timer("previous applications balances"):
            pos = self.get_pos_cash(self.DATA_DIRECTORY, num_rows)
            df = pd.merge(df, pos, on='SK_ID_CURR', how='left')
            print("Pos-cash dataframe shape: ", pos.shape)
            del pos; gc.collect()
            ins = self.get_installment_payments(self.DATA_DIRECTORY, num_rows)
            df = pd.merge(df, ins, on='SK_ID_CURR', how='left')
            print("Installments dataframe shape: ", ins.shape)
            del ins; gc.collect()
            cc = self.get_credit_card(self.DATA_DIRECTORY, num_rows)
        ...
        df = self.add_ratios_features(df)
        ...
        with self.timer("Run LightGBM"):
            feat_importance = self.kfold_lightgbm_sklearn(df, lgbm_categorical_feat)
            print(feat_importance)


    def add_ratios_features(self, df):
    ...
    def kfold_lightgbm_sklearn(self, data, categorical_feature = None):
        ...
        if not self.STRATIFIED_KFOLD:
            folds = KFold(n_splits= self.NUM_FOLDS, shuffle=True, random_state= self.RANDOM_SEED)
        else:
            folds = StratifiedKFold(n_splits= self.NUM_FOLDS, shuffle=True, random_state= self.RANDOM_SEED)
        ...
        for n_fold, (train_idx, valid_idx) in enumerate(folds.split(df[predictors], df['TARGET'])):
            ...
            params = {'random_state': self.RANDOM_SEED, 'nthread': self.NUM_THREADS}
            clf = LGBMClassifier(**{**params, **self.LIGHTGBM_PARAMS})
        ...
        if self.GENERATE_SUBMISSION_FILES:
            ...
            df.to_csv('oof{}.csv'.format(self.SUBMISSION_SUFIX), index=False)
            # Save submission (test data) and feature importance
            test[['SK_ID_CURR', 'TARGET']].to_csv('submission{}.csv'.format(self.SUBMISSION_SUFIX), index=False)
            mean_importance.to_csv('feature_importance{}.csv'.format(self.SUBMISSION_SUFIX), index=False)
        return mean_importance


    # ------------------------- APPLICATION PIPELINE -------------------------

    def get_train_test(self, path, num_rows = None):
        ...
        df['AGE_RANGE'] = df['DAYS_BIRTH'].apply(lambda x: self.get_age_label(x))
        ...
        df = self.do_median(df, group, 'EXT_SOURCES_MEAN', 'GROUP_EXT_SOURCES_MEDIAN')
        df = self.do_std(df, group, 'EXT_SOURCES_MEAN', 'GROUP_EXT_SOURCES_STD')
        df = self.do_mean(df, group, 'AMT_INCOME_TOTAL', 'GROUP_INCOME_MEAN')
        df = self.do_std(df, group, 'AMT_INCOME_TOTAL', 'GROUP_INCOME_STD')
        df = self.do_mean(df, group, 'CREDIT_TO_ANNUITY_RATIO', 'GROUP_CREDIT_TO_ANNUITY_MEAN')
        df = self.do_std(df, group, 'CREDIT_TO_ANNUITY_RATIO', 'GROUP_CREDIT_TO_ANNUITY_STD')
        df = self.do_mean(df, group, 'AMT_CREDIT', 'GROUP_CREDIT_MEAN')
        df = self.do_mean(df, group, 'AMT_ANNUITY', 'GROUP_ANNUITY_MEAN')
        df = self.do_std(df, group, 'AMT_ANNUITY', 'GROUP_ANNUITY_STD')

        # Encode categorical features (LabelEncoder)
        df, le_encoded_cols = self.label_encoder(df, None)
        df = self.drop_application_columns(df)
        return df


    def drop_application_columns(self, df):
    ...
    def get_age_label(self, days_birth):
    ...
    def get_bureau(self, path, num_rows= None):
        ...
        # One-hot encoder
        bureau, categorical_cols = self.one_hot_encoder(bureau, nan_as_category= False)
        # Join bureau balance features
        #bureau = bureau.merge(self.get_bureau_balance(path, num_rows), how='left', on='SK_ID_BUREAU')
        bb = self.get_bureau_balance(path, num_rows)
        ...
        agg_bureau = self.group(bureau, 'BUREAU_', self.BUREAU_AGG)
        # Active and closed loans aggregations
        active = bureau[bureau['CREDIT_ACTIVE_Active'] == 1]
        agg_bureau = self.group_and_merge(active,agg_bureau,'BUREAU_ACTIVE_',self.BUREAU_ACTIVE_AGG)
        closed = bureau[bureau['CREDIT_ACTIVE_Closed'] == 1]
        agg_bureau = self.group_and_merge(closed,agg_bureau,'BUREAU_CLOSED_',self.BUREAU_CLOSED_AGG)
        del active, closed; gc.collect()
        # Aggregations for the main loan types
        for credit_type in ['Consumer credit', 'Credit card', 'Mortgage', 'Car loan', 'Microloan']:
            type_df = bureau[bureau['CREDIT_TYPE_' + credit_type] == 1]
            prefix = 'BUREAU_' + credit_type.split(' ')[0].upper() + '_'
            agg_bureau = self.group_and_merge(type_df, agg_bureau, prefix, self.BUREAU_LOAN_TYPE_AGG)
            del type_df; gc.collect()
        # Time based aggregations: last x months
        for time_frame in [6, 12]:
            prefix = "BUREAU_LAST{}M_".format(time_frame)
            time_frame_df = bureau[bureau['DAYS_CREDIT'] >= -30*time_frame]
            agg_bureau = self.group_and_merge(time_frame_df, agg_bureau, prefix, self.BUREAU_TIME_AGG)
    ...
    def get_bureau_balance(self, path, num_rows= None):
        ...
        bb_processed = self.group_and_merge(bb, bb_processed, '', agg, 'SK_ID_BUREAU')
        del bb; gc.collect()
        return bb_processed

    # ------------------------- PREVIOUS PIPELINE -------------------------

    def get_previous_applications(self, path, num_rows= None):
        ...
        prev, categorical_cols = self.one_hot_encoder(prev, ohe_columns, nan_as_category= False)
        ...
        active_agg_df = self.group(active_df, 'PREV_ACTIVE_', self.PREVIOUS_ACTIVE_AGG)
        ...
        agg_prev = self.group(prev, 'PREV_', {**self.PREVIOUS_AGG, **categorical_agg})
        # Merge active loans dataframe on agg_prev
        agg_prev = agg_prev.merge(active_agg_df, how='left', on='SK_ID_CURR')
        del active_agg_df; gc.collect()
        # Aggregations for approved and refused loans
        agg_prev = self.group_and_merge(approved, agg_prev, 'APPROVED_', self.PREVIOUS_APPROVED_AGG)
        refused = prev[prev['NAME_CONTRACT_STATUS_Refused'] == 1]
        agg_prev = self.group_and_merge(refused, agg_prev, 'REFUSED_', self.PREVIOUS_REFUSED_AGG)
        del approved, refused; gc.collect()
        # Aggregations for Consumer loans and Cash loans
        for loan_type in ['Consumer loans', 'Cash loans']:
            type_df = prev[prev['NAME_CONTRACT_TYPE_{}'.format(loan_type)] == 1]
            prefix = 'PREV_' + loan_type.split(" ")[0] + '_'
            agg_prev = self.group_and_merge(type_df, agg_prev, prefix, self.PREVIOUS_LOAN_TYPE_AGG)
            del type_df; gc.collect()
        ...
        agg_dpd = self.group_and_merge(prev[prev['SK_ID_PREV'].isin(dpd_id)], agg_prev,
                                        'PREV_LATE_', self.PREVIOUS_LATE_PAYMENTS_AGG)
        del agg_dpd, dpd_id; gc.collect()
        # Aggregations for loans in the last x months
        for time_frame in [12, 24]:
            time_frame_df = prev[prev['DAYS_DECISION'] >= -30*time_frame]
            prefix = 'PREV_LAST{}M_'.format(time_frame)
            agg_prev = self.group_and_merge(time_frame_df, agg_prev, prefix, self.PREVIOUS_TIME_AGG)
            del time_frame_df; gc.collect()
        del prev; gc.collect()
        return agg_prev
    ...
    def get_pos_cash(self, path, num_rows= None):
        ...
        categorical_agg = {key: ['mean'] for key in categorical_cols}
        pos_agg = self.group(pos, 'POS_', {**self.POS_CASH_AGG, **categorical_agg})
    ...
    def get_installment_payments(self, path, num_rows= None):
        """ Process installments_payments.csv and return a pandas dataframe. """
        #pay = pd.read_csv(os.path.join(path, 'installments_payments.csv'), nrows= num_rows)
        pay = pd.read_csv(os.path.join(path, 'installments_payments.csv'))
        # Group payments and get Payment difference
        pay = self.do_sum(pay, ['SK_ID_PREV', 'NUM_INSTALMENT_NUMBER'], 'AMT_PAYMENT', 'AMT_PAYMENT_GROUPED')
        ...
        pay_agg = self.group(pay, 'INS_', self.INSTALLMENTS_AGG)
        ...
        for months in [36, 60]:
            ...
            pay_agg = self.group_and_merge(pay_recent, pay_agg, prefix, self.INSTALLMENTS_TIME_AGG)
        ...
        func = partial(self.trend_in_last_k_instalment_features, periods= self.INSTALLMENTS_LAST_K_TREND_PERIODS)
        g = self.parallel_apply(gp, func, index_name='SK_ID_CURR', chunk_size=10000).reset_index()
        pay_agg = pay_agg.merge(g, on='SK_ID_CURR', how='left')

        # Last loan features
        g = self.parallel_apply(gp, self.installments_last_loan_features, index_name='SK_ID_CURR', chunk_size=10000).reset_index()
        pay_agg = pay_agg.merge(g, on='SK_ID_CURR', how='left')
        return pay_agg


    def trend_in_last_k_instalment_features(self, gr, periods):
        gr_ = gr.copy()
        ...
        for period in periods:
            ...
            features = self.add_trend_feature(features, gr_period, 'DPD',
                                            '{}_TREND_'.format(period))
            features = self.add_trend_feature(features, gr_period, 'PAID_OVER_AMOUNT',
                                            '{}_TREND_'.format(period))
        return features


    def installments_last_loan_features(self, gr):
        gr_ = gr.copy()
        gr_.sort_values(['DAYS_INSTALMENT'], ascending=False, inplace=True)
        ...
        features = {}
        features = self.add_features_in_group(features, gr_, 'DPD',
                                        ['sum', 'mean', 'max', 'std'],
                                        'LAST_LOAN_')
        features = self.add_features_in_group(features, gr_, 'LATE_PAYMENT',
                                        ['count', 'mean'],
                                        'LAST_LOAN_')
        features = self.add_features_in_group(features, gr_, 'PAID_OVER_AMOUNT',
                                        ['sum', 'mean', 'max', 'min', 'std'],
                                        'LAST_LOAN_')
        features = self.add_features_in_group(features, gr_, 'PAID_OVER',
                                        ['count', 'mean'],
                                        'LAST_LOAN_')
        return features
    ...
    def get_credit_card(self, path, num_rows= None):
        ...
        cc, cat_cols = self.one_hot_encoder(cc, nan_as_category=False)
        ...
        # Aggregations by SK_ID_CURR
        cc_agg = cc.groupby('SK_ID_CURR').agg(self.CREDIT_CARD_AGG)
        ...
        for months in [12, 24, 48]:
            ...
            prefix = 'INS_{}M_'.format(months)
            cc_agg = self.group_and_merge(cc_recent, cc_agg, prefix, self.CREDIT_CARD_TIME_AGG)
        return cc_agg
    ...
    def do_mean(self, df, group_cols, counted, agg_name):
    ...
    def do_median(self, df, group_cols, counted, agg_name):
    ...
    def do_std(self, df, group_cols, counted, agg_name):
    ...
    def do_sum(self, df, group_cols, counted, agg_name):
    ...
    def one_hot_encoder(self, df, categorical_columns=None, nan_as_category=True):
    ...
    def label_encoder(self, df, categorical_columns=None):
    ...
    def add_features(self, feature_name, aggs, features, feature_names, groupby):
    ...
    def add_features_in_group(self, features, gr_, feature_name, aggs, prefix):
        for agg in aggs:
            if agg == 'sum':
    ...
    def add_trend_feature(self, features, gr, feature_name, prefix):
    ...
    def parallel_apply(self, groups, func, index_name='Index', num_workers=0, chunk_size=100000):
        if num_workers <= 0: num_workers = self.NUM_THREADS
        #n_chunks = np.ceil(1.0 * groups.ngroups / chunk_size)
        indeces, features = [], []
        for index_chunk, groups_chunk in self.chunk_groups(groups, chunk_size):
    ...
    def chunk_groups(self, groupby_object, chunk_size):
